...
sub copy_model_course {
    ...
    my $cmd       = [
        $full_path,                              '-r',
        "$webwork_dir/courses.dist/modelCourse", "$courses_dir"
    ];
    my ( $success, $error_message, $full_buf, $stdout_buf, $stderr_buf ) = run(
        command => $cmd,
        verbose => IPC_CMD_VERBOSE,
    ...
    if ($success) {
        ...
        warn
"Could not copy modelCourse/ to $courses_dir/. You'll have to copy this over manually: $error_message\n";
    }
}
...
sub write_launch_browser_script {
   ...
   #Remember that we used User::pwent which overrides the builtin pw* functions.
    ...
    print $out "$browser $url 2>/dev/null\n";
    close($out);
